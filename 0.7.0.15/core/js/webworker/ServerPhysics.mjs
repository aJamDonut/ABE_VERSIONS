...
class ServerPhysics {
	...
	newAvoid(box, currentVelocity, boids) {
		const AVOID_DISTANCE = Math.max(32, 128 * (2000 - Date.now() % 2000) / 100);
		let vecSeperation = this.Vector.create();

		let total = 0;

		for (let boid of boids) {
			if (boid.data.dead || boid.data.do === "becarried") continue;
			boid = boid.body;
			if (boid === box) continue; //Skip self
		...
		const multi = 50//Math.max(50, 300 * (5000 - Date.now() % 5000) / 100);
	...
	processDesiredVelocity(item, body, velData) {
		...
		if (item.baseClass !== "BaseLife" || this.shouldFollowPath(item)) {
			...
			desired = this.newAvoid(body, desired, Object.values(this.index.getIndex("life")));

			if(isNaN(desired.x) || isNaN(desired.y)) {
				console.error("[ABE] BAD VECTOR FROM VELTODATA", item, body, velData, desired);
				return;
			}
		} else {
			...
			if(isNaN(desired.x) || isNaN(desired.y)) {
				console.error("[ABE] BAD VECTOR FROM VELTOTARGET", item, body, velData, desired);
				return;
			}
