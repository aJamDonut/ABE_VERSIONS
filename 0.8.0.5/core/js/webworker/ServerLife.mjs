...
class ServerLife extends mixin(RelativePositionsServer) {
	...
	physicsTick() {
		...
		let dist = this.helper.dist(
			{x: this.body.position.x + (64 * 3) / 2, y: this.body.position.y + (64 * 3) / 2},
	...
	addStat(name) {
		if (!this.data.levels[name]) {
			this.data.levels[name] = {xp: 0, level: 1, lastXP: 0};
		}
	...
	hurt(amount, type, options) {
		...
		*/
		...
		if (toughness < 50) {
			if (this.data.stats[bodyPart] < koLevel && this.helper.rng(0, 3) === 2) {
	...
	upkeep(stat, amount) {
		this.hurt(amount, "none", {bodyPart: stat});
		this.capStats();
	}
