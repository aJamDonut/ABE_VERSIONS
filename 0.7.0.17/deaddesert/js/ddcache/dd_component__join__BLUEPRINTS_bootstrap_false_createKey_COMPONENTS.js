(function () {
  ...
  _BLUEPRINTS.COMPONENTS.hud_settings_preferences = {
    "getElement": function () {
      ...
      pane.x = 40;
      let booleans = ['showTut', 'damage', 'blood'];
      let descs = ['Enables showing the tutorial', 'Floating damage numbers`nDisable if slow framerate', 'Blood`nDisable if slow framerate or dislike'];
      let padding = 5;
  ...
  _BLUEPRINTS.COMPONENTS.hud_settings_panel = {
    "getElement": function (options) {
      ...
      options.tabIndex = options.tabIndex || 0;
      const tabContainer = game.render.component('component_container_tabs', {
        tabNames: ['Preferences', 'Graphics', 'Controls'],
        tabComponents: [game.render.component('hud_settings_preferences'), game.render.component('hud_settings_graphics'), game.render.component('hud_settings_preferences')],
        w: options.w || 800,
        h: options.h || 500,
        tabIndex: options.tabIndex
      });
  ...
  _BLUEPRINTS.COMPONENTS.hud_settings_graphics = {
    "getElement": function () {
      ...
      let descs = ['Advanced filters`nDisable for more FPS', 'Particles`nEffects animations like gun shrapnel', 'Soft Loading`nEnable if game jerks between areas'];
      let padding = 5;
      let note = game.render.text('Note: You may have to restart for changes to take effect', 'ingame-label');
      note.x = 5;
      pane.addChild(note);
