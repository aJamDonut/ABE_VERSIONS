...
bootStrap.push(function () {
  function addFloor(layer, across, down, name = 'Floortile', desc = _LANG.WAITING_FOR_DESCRIPTION) {
  ...
  addFloor(1, 9, 22, 'Red Royale');
  addFloor(1, 9 + 3, 22, 'Orange Royale');
  addFloor(1, 9 + 3 + 3, 22, 'Blue Royale');
  addFloor(1, 9 + 3 + 3 + 3, 22, 'Green Royale');
  addFloor(1, 1, 29, 'Tarbrick');
  addFloor(1, 2, 29, 'Redrow');
  addFloor(1, 3, 29, 'Vertrow');
  addFloor(1, 4, 29, 'Floor dark');
  addFloor(1, 1, 30, 'Hard gravel');
  addFloor(1, 2, 30, 'Red Hex');
  addFloor(1, 3, 30, 'Colored Hex');
  addFloor(1, 4, 30, 'Grayth Hex');
  addFloor(1, 5, 30, 'Whythe Gray');
  addFloor(2, 5, 29, 'Shadow Light');
  addFloor(2, 6, 29, 'Shadow Dark');
  addFloor(2, 7, 29, 'Pt Shadow Light');
  addFloor(2, 8, 29, 'Pt Shadow Dark');
  addFloor(1, 1, 31, 'Plain wood');
  addFloor(1, 2, 31, 'Ash wood');
  addFloor(1, 3, 31, 'Sider wood');
  addFloor(1, 4, 31, 'Bluth wood');
  addFloor(1, 5, 31, 'Plumpth wood');
  addFloor(1, 6, 31, 'Whythe wood');
  addFloor(1, 24, 7, 'Red rug');
  addFloor(1, 27, 7, 'Orange rug');
  addFloor(2, 6, 14, 'Box TL');
  addFloor(2, 22, 14, 'Red box');
  game.world.objectTemplates.walls.push({
    name: 'buildroom',
    readName: 'Dark Sand',
    codename: 'floor',
...
bootStrap.push(function () {
  ...
  game.util.drawZoneTool = zone => {
    game.util.drawTool = {
      points: [],
      zone: zone || game.activeDrawZone || 0,
      isActive: true,
  ...
  function showZones() {
    cleanup();
    if (!game.grid.chunkTree["0_0.json"].meta || !game.grid.chunkTree["0_0.json"].meta.zones) return;
    let polygons = game.grid.chunkTree["0_0.json"].meta.zones;
    for (let i = 0; i < polygons.length; i++) {
      ...
      graphics.endFill();
      graphics.alpha = 0.5;
      game.render.lifeLayer.addChild(graphics);
      drawnZones.push(graphics);
    }
  }
  function cleanup() {
    for (let circle of circles) {
      if (circle._geometry) circle.destroy();
    }
    for (let zone of drawnZones) {
      if (zone._geometry) zone.destroy();
    }
    drawZones = [];
    circles = [];
  ...
  function commit() {
    game.grid.clearZoneCache();
    if (!game.grid.chunkTree["0_0.json"].meta) {
      game.grid.chunkTree["0_0.json"].meta = {};
    }
    const meta = game.grid.chunkTree["0_0.json"].meta;
    if (!meta.zones) {
      meta.zones = [];
    }
    meta.zones.push({
      zone: game.util.drawTool.zone,
      points: game.util.drawTool.points
    });
  }
  ...
  game.util.drawZoneTool.onDown = () => {
    ...
    game.util.drawTool.points.push({
      x: 0 + game.snapToGrid(game.mouseX),
      y: 0 + game.snapToGrid(game.mouseY)
    });
