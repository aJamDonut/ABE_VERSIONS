...
class ServerPhysics {
	...
	newAvoid(box, currentVelocity, boids) {
		...
		return this.Vector.add(currentVelocity, this.Vector.mult(vecSeperation, 150));
	}
	...
	getVelDataVelocity(item, body, velData) {
		if (velData !== undefined) {
			...
			return this.Vector.mult(this.Vector.neg(this.Vector.normalise(vecS)), 0.5 * speed);
		}
	...
	shouldFollowPath(item) {
		...
		if (this.helper.dist(item, target) > 64 * 3) {
			return true;
		}
